...
- Real time activity feed for latest actions by users and changes to pets (bottom right).
- Slideshow showing one of the pets at random. Changes every few seconds. (bottom left). <br>
  For logged-n users, clicking the image links to the pet's page.
- Our mascots, the hyper-laser cat and dog (top right and left).
...
A Login/Sign-up. Button for it is on the navigation bar. <br>
